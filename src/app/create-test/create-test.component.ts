...
export class CreateTestComponent implements OnInit {
    ...
    this.route.queryParams.subscribe((params: any) => {
      ...
      if (params) {
        ...
        if (params.isEditing) {
          ...
          let test: Test = JSON.parse(params.test);

          this.title = test.title;
          this.passMarks = test.pass_marks,
          this.description = test.description;
          this.timeinseconds = test.time;
          this.timeInString = this.changeSecondsToTime(test.time);
          this.withoutTime = test.is_without_time;
          this.isActive = test.isActive;
          this.negativeMarking = test.negativeMarking;
  ...
  async submit() {
    ...
    if (await this.checkErrors()) {
      ...
      this.http.post<any>(Params.SERVICE_BASE_URL + Params.EXAM_SERVICE_URL_SUFFIXS.CREATE_TEST, body, { headers }).subscribe({
        next: data => {
          ...
          if (data.success) {
            ...
            alert('🚀 Test added successfuly!')

            this.location.back();

          } else {

            alert('Test adding failed!')
  ...
  async update() {
    ...
    if (await this.checkErrors()) {
      ...
      const options: any = {
        ...
        headers: headers,
        params: { "examId": await this.getEditingTest()._id, "examType": "test" },
        // responseType: 'text'
      ...
      this.http.post<any>(Params.SERVICE_BASE_URL + Params.EXAM_SERVICE_URL_SUFFIXS.UPDATE_EXAM, body, options).subscribe({
        next: (data: any) => {
          ...
          console.log(data)
          alert('🚀 Test updated successfuly!')
